...
P2-5（權重提示）→ P3-6（決策回顧統計）`，`P4+-7`（後端化／LLM agent）待使用者
明確決定要不要跨出純前端範疇後再展開細規格。
