...
#### 7.5 五個生活面向 LLM agent `[ ]`——**一次做一個，第一個建議是 Reflection**
...
- **Reflection agent（建議第一個做）**：呼應現有「反思提醒」卡片
  （`renderOverview` 裡目前是挑 `impact*relevance*unique` 最高的一筆事件、
  套固定文字模板）。改成：把目前的 `goals`/`events`/`actions`/`decisions`
  整包送給 7.4 的代理，讓 Claude 產生客製化的反思提示，取代現在的固定模板。
  選它當第一個的原因：現有 UI 已經有掛載點（`#reflection-title`/
  `#reflection-text`），不需要新增畫面，最小成本驗證「LLM 整合有沒有價值」。
- Career／Finance／Health agent：規格待這個 pilot 跑完、確認方向後再展開
- Research agent：用途未定，暫不展開

**涉及檔案/模組**：`app.js` 的 `renderOverview()`（改成非同步呼叫 7.4 代理）
...
`P4+-7`（後端化／LLM agent）已拆解成 7.1–7.8 八個子項目，使用者已回答關鍵
問題（單人自用／Claude API／Research agent 待定／agent 一個一個做），AI 也
針對「要不要保留純前端模式」給了評估：**建議先只做 7.4（Claude API 輕量
代理）+ 7.2（共用密鑰防護，隨 7.4 一起做）+ 7.5 的第一個 agent（建議
Reflection），把 7.1／7.3／7.6／7.7／7.8 全部暫緩**，理由詳見 7. 開頭的
「AI 評估建議」段落。**目前仍尚未實作任何一項**，等使用者看過這個修訂後的
順序、確認要開始，再從 `7.4 → 7.2 → 7.5（Reflection）` 這個最小可行路徑
動工。
