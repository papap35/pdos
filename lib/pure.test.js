...
import { score, esc, formatDate, isValidData, migrateActionGoals, buildReflectionPrompt } from './pure.js';
...
describe('buildReflectionPrompt', () => {
  const data = {
    goals: [{ id: 1, title: 'Goal A', category: 'Career', priority: 90, progress: 40, deadline: '2027-01-01' }],
    events: [
      { date: '2026-07-01', title: 'Old event', note: 'old note' },
      { date: '2026-07-10', title: 'New event', note: 'new note' },
    ],
    actions: [
      { title: 'Open action', goalId: 1, done: false },
      { title: 'Done action', goalId: 1, done: true },
    ],
    decisions: [{ date: '2026-07-05', question: 'Question?', recommended: 'Option A' }],
  };

  it('includes goal, event, open-action, and decision content', () => {
    const prompt = buildReflectionPrompt(data);
    expect(prompt).toContain('Goal A');
    expect(prompt).toContain('New event');
    expect(prompt).toContain('Open action');
    expect(prompt).toContain('Question?');
  });
  it('excludes completed actions', () => {
    expect(buildReflectionPrompt(data)).not.toContain('Done action');
  });
  it('sorts events and decisions newest-first', () => {
    const prompt = buildReflectionPrompt(data);
    expect(prompt.indexOf('New event')).toBeLessThan(prompt.indexOf('Old event'));
  });
  it('resolves an action goalId to the current goal title, not a stale snapshot', () => {
    const renamed = { ...data, goals: [{ ...data.goals[0], title: 'Renamed Goal' }] };
    expect(buildReflectionPrompt(renamed)).toContain('Renamed Goal');
  });
  it('falls back to a placeholder when the referenced goal no longer exists', () => {
    const orphaned = { ...data, goals: [] };
    expect(buildReflectionPrompt(orphaned)).toContain('（目標已刪除）');
  });
  it('shows placeholders for empty sections instead of crashing', () => {
    const empty = { goals: [], events: [], actions: [], decisions: [] };
    expect(() => buildReflectionPrompt(empty)).not.toThrow();
    expect(buildReflectionPrompt(empty)).toContain('（無）');
  });
  it('treats a missing decisions field as empty rather than throwing', () => {
    const { decisions, ...withoutDecisions } = data;
    expect(() => buildReflectionPrompt(withoutDecisions)).not.toThrow();
  });
});
